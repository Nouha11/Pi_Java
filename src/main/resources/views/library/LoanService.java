...
public class LoanService {
    ...
    public void ajouter(Loan l) throws SQLException {
        String sql = "INSERT INTO loan (user_id, book_id, library_id, status, requested_at) VALUES (?,?,?,?,?)";
    ...
    public void supprimer(int id) throws SQLException {
        PreparedStatement ps = cnx.prepareStatement("DELETE FROM loan WHERE id=?");
        ps.setInt(1, id);
        ps.executeUpdate();
    ...
    public void approuver(int loanId) throws SQLException {
        ...
        PreparedStatement ps = cnx.prepareStatement(
                "UPDATE loan SET status=?, approved_at=? WHERE id=?");
    ...
    public void rejeter(int loanId, String reason) throws SQLException {
        PreparedStatement ps = cnx.prepareStatement(
                "UPDATE loan SET status=?, rejection_reason=? WHERE id=?");
    ...
    public void marquerActif(int loanId) throws SQLException {
        ...
        PreparedStatement ps = cnx.prepareStatement(
                "UPDATE loan SET status=?, start_at=?, end_at=? WHERE id=?");
    ...
    public List<Loan> afficher() {
        ...
        String sql = "SELECT l.*, b.title as book_title, u.email as user_name, lib.name as library_name " +
                ...
                "LEFT JOIN library lib ON l.library_id=lib.id " +
    ...
    private int countActiveLoans(int userId) throws SQLException {
        PreparedStatement ps = cnx.prepareStatement(
                "SELECT COUNT(*) FROM loan WHERE user_id=? AND status=?");
        ps.setInt(1, userId);

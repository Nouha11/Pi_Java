...
public class BookService {
    ...
    // ─────────────────────────────────────────────
    //  CONNECTION — always get a fresh valid connection
    // ─────────────────────────────────────────────
    ...
    private Connection getCnx() {
        try {
            ...
            System.err.println("BookService connection error: " + e.getMessage());
            return null;
        }
    }
    ...
    public void ajouter(Book b) throws SQLException {
        String sql = "INSERT INTO books (title, author, isbn, price, type, cover_image, pdf_url) VALUES (?,?,?,?,?,?,?)";
    ...
    public void modifier(Book b) throws SQLException {
        String sql = "UPDATE books SET title=?, author=?, isbn=?, price=?, type=?, cover_image=?, pdf_url=? WHERE id=?";
        PreparedStatement ps = getCnx().prepareStatement(sql);
        ps.setString(1, b.getTitle().trim());
        ps.setString(2, b.getAuthor().trim());
        ps.setString(3, b.getIsbn());
    ...
    public List<Book> afficher() {
        ...
        try {
            ResultSet rs = getCnx().createStatement().executeQuery("SELECT * FROM books ORDER BY title");
